...
const hiddenClientID = process.env['id']
const hiddenClientSecret = process.env['secret']
...
app.get('/name', isAuthenticated, async function(request,response){
  let results = await dbo.getProfile(request.user.userid);
  let r = {name: results.name};
  response.json(r);
});
...
app.get('/reminder', isAuthenticated, async function(request, response, next) {
  ...
  if (result != null){
    ...
    response.send(act.Activity(result, request.user.userid));
  } else {
...
function gotProfile(accessToken, refreshToken, profile, done) {
    ...
    dbo.insertProfile(profile.name.givenName, profile.id);
...
passport.deserializeUser((id, done) => {
    console.log("deserializeUser. Input is:", id);
    // here is a good place to look up user data in database using
    // dbRowID. Put whatever you want into an object. It ends up
    // as the property "user" of the "req" object. 
    let userData = {userData: "data from user's db row goes here", userid: id};
    done(null, userData);
});
