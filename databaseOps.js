...
const {db, pdb} = require('./sqlWrap');

// our activity verifier
const act = require('./activity');

// SQL commands for ActivityTable
const insertDB = "insert into ActivityTable (activity, date, amount, id) values (?,?,?,?)"
...
const getMostRecentPrevPlannedDB = "SELECT rowIdNum, activity, MAX(date), amount FROM ActivityTable WHERE amount <= 0 and date BETWEEN ? and ? and id = ?";
const getMostRecentDB = "SELECT MAX(rowIdNum), activity, date, amount, where id = ? FROM ActivityTable";
const getPastWeekByActivityDB = "SELECT * FROM ActivityTable WHERE activity = ? and date BETWEEN ? and ? and id = ? ORDER BY date ASC";
const addProfile = "insert into Profile (name, id) values (?,?)";
const findProfile = "select * from Profile where id = ?";
const listProfiles = "select * from Profile"
 ...
 */
async function post_activity(activity, id) {
  try {
 ...
 */
async function get_most_recent_planned_activity_in_range(min, max, id) {
  try {
    let results = await db.get(getMostRecentPrevPlannedDB, [min, max , id]);
 ...
 */
async function get_similar_activities_in_range(activityType, min, max, id) {
  try {
    let results = await db.all(getPastWeekByActivityDB, [activityType, min, max, id]);
    return results;
  }
  catch (error) {
    console.log(error);
...
async function insertProfile(name, id){
  try{
    let check = await pdb.all(findProfile, id);
    if(check.length === 0){
      await pdb.run(addProfile, [name, id])
    }
  }
  catch(error){
    console.log(error);
...
async function getProfile(id){
  try{
    let result = await pdb.get(findProfile, id);
    return result;
  }
  catch(error){
    console.log(error);
